...
model = Sequential()
model.add(Conv2D(imgSize/2,3,padding="same", activation="relu", input_shape=(imgSize,imgSize,3)))
model.add(MaxPool2D())
model.add(Conv2D(imgSize/2, 3, padding="same", activation="relu"))
model.add(MaxPool2D())
model.add(Flatten())
model.add(Dense(imgSize*2,activation="relu"))
...
optiSGD = SGD(learning_rate=0.0001)
model_SGD.compile(optimizer = optiSGD , loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True) , metrics = ['accuracy'])
resultSGD = model_SGD.fit(X_train,X_label,epochs = 5, validation_data = (X_test, test_label))
...
optiAdam = Adam(learning_rate=0.0001)
model_Adam.compile(optimizer = optiAdam , loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True) , metrics = ['accuracy'])

resultAdam = model_Adam.fit(X_train,X_label,epochs = 5 , validation_data = (X_test, test_label))
